...
<section>
	...
	<jsp:include page="../include/basicMyPageSideBar.jsp"/>
		...
		<div class="Content">
			<form action="../delete/bmWidhdraw.jsp" method="POST" name="frm">
			<h2>탈퇴안내</h2>
			<p class="title">회원을 탈퇴하기전에 아래의 사항을 꼭 확인해주세요.</p>
			<br>
			<ul class="withdraw">
				<li>사용하고 계신 아이디(<span class="point"><%=session.getAttribute("basicId") %></span>)는 탈퇴할경우 재사용과 복구가 불가능합니다.</li>
			...
			<div class="accept">
				...
				<br>
					<h3>개인사유로 인한 탈퇴시 포인트 소멸안내 </h3>
				<ul>
					<li>개인적인 사유로 인한 탈퇴의 경우 포인트는 소멸되며 복구가 어렵습니다.</li>
					<li>탈퇴 후에는 회원정보가 삭제되며 포인트에 대한 금전적인 환불이 불가하며, <span class="point2">포인트에 대한 권한이 없어집니다.</span></li>			
					<li class="accept">위의 안내사항을 모두 확인하였으며,이에 동의합니다.<input type="checkbox" value="Y" name="accept" id="accept"></li>
				</ul>			
			</div>
			<div class="btn">
				<input type="submit" value="회원탈퇴" class="unregister" onclick="return bmWithdraw()">
				</div>
				</form>	
		</div>
	</div>
